...
Status: **GPU runtime and combined application gate verified; instance stopped after testing** on 2026-09-23. The deployed instance is Brev GCP NVIDIA L4 `iyfz2jzfd` (`atlas-hackalem-gcp`), 24 GiB VRAM and 129 GB disk. Brev displayed $0.87/hour while running and $0.02/hour for retained storage after it reached STOPPED at approximately 11:30 UTC. The full Brev Compose stack (database, API, web and private GPU specialist) built and became healthy; migrations and deterministic synthetic seed completed. The successful combined workflow and its actual event-decision effect are recorded in [combined verification](../../docs/review/combined.md).
...
- Ignored evidence record: `infra/brev/evidence/verification-ba8c3ebf-1bde-4e0f-af73-dfa304d6b5ca.json`. It was copied from Brev to the local ignored evidence directory before shutdown and is not committed. Regenerate it after any restart, image/model change, or runtime change; prior verification must not be treated as current after that point.
...
After GPU work, stop the instance in Brev and confirm its state and displayed rate. Container shutdown alone leaves compute charges active. This instance reached STOPPED; Brev then displayed $0.02/hour for retained storage rather than the $0.87/hour running rate. Restarting it requires fresh GPU verification before claiming another live GPU run.
