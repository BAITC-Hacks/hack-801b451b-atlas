...
| T01 | `BackendTools.inspect(context): Promise<ToolResult<Inspection>>` | `context:{signal:AbortSignal}`; closure binds validated dataset/warehouse/category | `Inspection` | DATA_GAP, CANDIDATE_LIMIT, CALCULATION_FAILED | Ivan |
| T02 | `BackendTools.calculate(input, context): Promise<ToolResult<Calculation>>` | `{specialist:SpecialistReport|null}`, plus context; report from validated GPU inference cache only | `Calculation` with immutable numeric rows | DATA_GAP, CANDIDATE_LIMIT, CALCULATION_FAILED | Ivan |
