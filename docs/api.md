...
| H09 | POST `/runs/:runId/quote` | `QuoteRunBody` below | 200 `{quote:OrderQuote}` | 400 validation/currency mismatch; 404 run; 409 stale revision; 503 DB | Ali (assigned API/contracts) |
...
## Order quote and budget check (H09)

`QuoteRunBody` is strict JSON: `{expectedRevision:integer>=1, currency?:three-letter code, prices:[{sku:Id,unitPriceMinor:nonnegative safe integer}], budgetMinor?:nonnegative safe integer}`. Price entries are unique and may be omitted only for zero-`finalQty` lines. The optional `currency`, if sent, must match the stored dataset currency case-insensitively; mismatch returns 400 `VALIDATION_ERROR` with issue `{path:'currency',code:'CURRENCY_MISMATCH'}`. The response always carries the dataset currency. Unknown SKUs, duplicate prices, missing prices for positive-quantity lines, invalid amounts and totals above `Number.MAX_SAFE_INTEGER` return 400. A stale `expectedRevision` returns 409 `REVISION_CONFLICT`.

`OrderQuote` is `{runId:Id,revision:integer,currency:string,lines:[{sku:Id,finalQty:Qty,unitPriceMinor:nonnegative safe integer|null,lineTotalMinor:nonnegative safe integer}],totalMinor:nonnegative safe integer,budgetMinor:nonnegative safe integer|null,remainingMinor:nonnegative safe integer|null,overageMinor:nonnegative safe integer|null,withinBudget:boolean|null}`. With no budget, the last four budget fields are null. With a budget, `remainingMinor=max(0,budgetMinor-totalMinor)`, `overageMinor=max(0,totalMinor-budgetMinor)`, and `withinBudget=(totalMinor<=budgetMinor)`. A zero-quantity line contributes zero, whether priced or not.

All prices, totals and budgets are exact integer **minor units of the dataset currency** supplied by the operator; the server performs no rounding or floating-point money arithmetic. Clients convert displayed major-unit amounts to valid minor-unit integers before calling H09. Historical sale `unitPrice` is not used as a supplier purchase price. H09 reads the stored run and dataset, works for draft or approved runs, changes neither, and does not alter H06 approval or H07 export behavior. It is a quote/check, not an authorized company budget or an order to a supplier.
