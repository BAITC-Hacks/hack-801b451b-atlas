...
API smoke scripts use the known test dataset returned by seed/import, not a hardcoded production dataset ID. Run/output IDs captured from actual responses. `openai-flow.ts` may pass with GPU absent but must report it; `combined-flow.ts` requires real OpenAI and GPU success, fresh evidence and material effect. Integration harness may directly import API tool factory through a test-only relative import; runtime AI package must not import API/DB or introduce dependency cycles.
